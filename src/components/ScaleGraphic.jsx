...
const ScaleGraphic = ({ keyNote, scaleType }) => {
      ...
      {items.map((item, i) => (
        <React.Fragment key={i}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
            <span style={{ fontWeight: 'bold', fontSize: '1rem', color: 'var(--color-primary, #646cff)' }}>{item.note}</span>
            <span style={{ fontSize: '0.7rem', color: 'var(--color-text-dim, #888)' }}>{item.intervalName}</span>
          </div>
